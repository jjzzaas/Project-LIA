(()=>{
  ...
  function renderMomoService(){
    mount(`<main class="screen lodging"><section class="box"><div class="text">처음 보는 여자아이가—<br><br>바로 옆에서 자고 있었다.</div></section><div class="hint">터치하여 계속</div><div class="version">Ver. 1.9</div></main>`,()=>app.firstElementChild.onclick=next);
  }

  function renderMomoAwake(scene){
    mount(`<main class="screen lodging"><section class="box"><div class="speaker">${scene.speaker}</div><div class="text">${scene.text}</div></section><div class="hint">터치하여 계속</div><div class="version">Ver. 1.9</div></main>`,()=>app.firstElementChild.onclick=next);
  }

  function renderExit(scene,screenType,label){
    mount(`<main class="screen ${screenType}"><button id="sceneExit" aria-label="${label}" style="${exitButtonStyle}">← ${label}</button><section class="box"><div class="text">${scene.text}</div></section><div class="version">Ver. 1.9</div></main>`,()=>document.getElementById('sceneExit').onclick=next);
  }

  function renderLobbyPause(scene){
    mount(`<main class="screen main-lobby"><header class="lobby-top"><div class="level-badge">Lv. ${state.level}</div><div class="stamina"><span>활력도</span><div><i></i></div><b>100 / 100</b></div><button class="mission-btn">임무</button></header><section class="hero-zone"><div class="hero-placeholder">HEROINE</div><div class="speech"><strong>${scene.speaker||'모모'}</strong><p>${scene.text.replace(/\n/g,'<br>')}</p></div></section><nav class="tiles"><button class="tile" disabled><span>01</span><strong>길드</strong></button><button class="tile" disabled><span>02</span><strong>숙소</strong></button><button class="tile" disabled><span>03</span><strong>잠김</strong></button>${[4,5,6].map(n=>`<button class="tile" disabled><span>0${n}</span><strong>잠김</strong></button>`).join('')}</nav><div class="hint">터치하여 계속</div><div class="version">Ver. 1.9</div></main>`,()=>app.firstElementChild.onclick=next);
  }
